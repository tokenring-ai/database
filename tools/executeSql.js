...
export async function execute({ databaseName, sqlQuery, queryParams }, registry) {
  ...
  try {
   ...
   return await resource.executeSql(sqlQuery, executionParams);
    ...
    return { error: `Failed to execute SQL query via resource: ${error.message}` };
  }
}

export const description = "Executes an arbitrary SQL query on a database using the DatabaseResource. WARNING: Use with extreme caution as this can modify or delete data.";
...
export const parameters = z.object({
  ...
  queryParams: z.object({}).passthrough().optional().describe("Optional: Parameters for the SQL query, for prepared statements or specific connection needs.")
});
